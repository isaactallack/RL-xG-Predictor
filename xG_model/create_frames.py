...
def create_lines(group, team1_ops, team2_ops):
  ...
  #if '76561198845707031' or '76561198021360335' in group['player id'].values:
  if group['player id'].astype(str).str.contains('76561198845707031').any() or group['player id'].astype(str).str.contains('76561198021360335').any():
...
def create_dfs():
    ...
    for group in groups:
        #if not '76561198845707031' or '76561198021360335' in pd.DataFrame(group[1])['player id'].values:
        if not pd.DataFrame(group[1])['player id'].astype(str).str.contains('76561198845707031').any() and not pd.DataFrame(group[1])['player id'].astype(str).str.contains('76561198021360335').any():
            bad_groups += [group[0]]
    ...
    for group in groups:
        df_teams = pd.concat([df_teams, create_lines(group, team1_ops, team2_ops)])
    ...
    combined.to_csv('data/raw_train.csv', index=False)
